...
const CommentModal = ({ selectedPost, onClose } : CommentModalProps ) => {
    ...
    const { commentText, createComment, isCreatingComment, setCommentText} = useComment();
            ...
            <ScrollView className='flex-1'>
                <View className='border-b border-gray-100 bg-white p-4'> 
                    <View className='flex-row'>
                        <Image source={{ uri : selectedPost.user.dp }} 
                ...
                {selectedPost.comments.map((comment) => (
                    <View key={comment._id} className='border-b border-gray-100 bg-white p-4'>
                        <View className='flex-row'>
                            ...
                            <View className='flex-1'>
                                <View className='flex-row items-center mb-1'>
                                    <Text className='font-bold text-gray-900 mr-1'>{comment.user.firstName} {comment.user.lastName}
                                    </Text>
